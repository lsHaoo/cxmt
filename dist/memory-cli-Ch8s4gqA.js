...
import { it as parseDurationMs, j as loadConfig, ln as splitShellArgs } from "./auth-profiles-BxNoHpB2.js";
...
async function getMemorySearchManager(params) {
	...
	if (resolved.backend === "qmd" && resolved.qmd) {
		...
		try {
			const { QmdMemoryManager } = await import("./qmd-manager-Do_K36uy.js");
			...
			if (primary) {
				...
				const wrapper = new FallbackMemoryManager({
					...
					fallbackFactory: async () => {
						const { MemoryIndexManager } = await import("./manager-gE13wutQ.js").then((n) => n.t);
